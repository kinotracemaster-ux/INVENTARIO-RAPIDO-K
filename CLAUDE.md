...
- **Sub-módulo Shopify · POEDAGAR** (21 sep 2026): CSV actualizado (stock + precio opcional) y Reporte (.xlsx) con los SKU nuevos clasificados (Marca/Accesorio/Familia — pedido explícito de Kino). Verificado a mano con un export real: CSV 541/541 filas OK celda por celda, reporte 2.660 SKU nuevos clasificados idéntico contra cálculo independiente. **Pendiente:** sin test automatizado en `test/run.py` (el CSV real de Shopify no se subió al repo, es data de Kino); sin drag&drop (solo botón); sin tabla "antes → después" fila por fila, solo resumen. Detalle completo: MEMORIA_PROYECTO.md §9.
