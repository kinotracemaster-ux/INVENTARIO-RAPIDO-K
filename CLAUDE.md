...
- **Sub-módulo Shopify · POEDAGAR** (21 sep 2026): se sube el CSV que exporta Shopify, se baja un **Excel** actualizado (stock + precio opcional) — Kino lo convierte a CSV a mano para resubirlo. Verificado a mano con un export real: 541/541 filas OK celda por celda. Se probó y luego se sacó un reporte con SKU nuevos clasificados (Kino lo pidió y después pidió sacarlo el mismo día — ver MEMORIA_PROYECTO.md §9 si se vuelve a pedir). **Pendiente:** sin test automatizado en `test/run.py` (el CSV real de Shopify no se subió al repo, es data de Kino); sin drag&drop (solo botón); sin tabla "antes → después" fila por fila, solo resumen. Detalle completo: MEMORIA_PROYECTO.md §9.
