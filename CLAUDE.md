...
- **Sub-módulo Shopify · POEDAGAR** (21 sep 2026): se sube el CSV que exporta Shopify, se baja un **Excel** actualizado (stock + precio opcional) — Kino lo convierte a CSV a mano para resubirlo. Los SKU nuevos (del SYS, sin usar en ese archivo) se muestran en una lista aparte en pantalla, y hay un botón extra para bajarlos como filas nuevas al final del Excel (SKU/Handle/Título/Stock/Precio ya cargados, `Published=false`/`Status=draft` para que no se publiquen solas, el resto vacío para completar a mano) — nunca se mezclan con el botón principal. Verificado a mano con un export real: Excel principal 541/541 filas OK celda por celda; Excel "con nuevos" 3.201 filas (541 + 2.660 nuevas), todo coincide contra cálculo independiente. Antes hubo un reporte (.xlsx) con SKU nuevos clasificados por Marca/Accesorio — Kino lo pidió sacar ese mismo día, ver historial en MEMORIA_PROYECTO.md §9. **Pendiente:** sin test automatizado en `test/run.py` (el CSV real de Shopify no se subió al repo, es data de Kino); sin drag&drop (solo botón); sin tabla "antes → después" fila por fila para los que cruzan, solo resumen. Detalle completo: MEMORIA_PROYECTO.md §9.
