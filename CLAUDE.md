...
Toda la lógica vive en `sys-a-kyte/app/sys-a-kyte.html` (funciones clave: `leerSys`, `leerTabla`, `procesar`, `libroHoja`/`libroReporte`, estado en el objeto `S`, para Kyte; `leerShopify`, `procesarShopify`, `xlsxShopify`/`xlsxShopifyConNuevos`/`filaNuevaShopify`, estado en `S.shop`, para el sub-módulo de Shopify — MEMORIA_PROYECTO.md §9). Flujo para cambiar lógica: editar ese archivo → `build_index.py` → `test/run.py`.
...
- **Sub-módulo Shopify · POEDAGAR** (21 sep 2026): se sube el CSV que exporta Shopify, se baja un **Excel de una sola hoja, "POEDAGAR ACTUALIZADA"** (`HOJA_SHOPIFY` en el código; también nombra el archivo descargado) — Kino lo convierte a CSV a mano para resubirlo. **Única fuente: la hoja "POEDAGAR" de la tabla de Kyte** (`sysDeHoja()`/`calcularShopify()`), no el SYS completo — si esa hoja no existe en la tabla, el sub-módulo avisa y no deja subir nada. **Regla de precio confirmada por Kino:** Kyte = SYS × 2 siempre; Shopify POEDAGAR = ese precio de Kyte + 40 % = SYS × 2,8 (de ahí el multiplicador por defecto). Los SKU nuevos (del SYS acotado a POEDAGAR, sin usar en ese archivo) se muestran en una lista aparte en pantalla, y hay un botón extra para bajarlos como filas nuevas al final del Excel (SKU/Handle/Título/Stock/Precio ya cargados, `Published=false`/`Status=draft` para que no se publiquen solas, el resto vacío para completar a mano) — nunca se mezclan con el botón principal. Verificado a mano con un export real acotado a la hoja POEDAGAR (478 artículos): 418/541 cruzan, Excel principal 541/541 filas OK celda por celda, Excel "con nuevos" 594 filas (541 + 53, todas POEDAGAR reales), todo contra cálculo independiente. Antes de acotar a la hoja POEDAGAR daba 2.660 "nuevos" de cualquier marca — era un bug, Kino lo reportó con captura. Antes también hubo un reporte (.xlsx) con SKU nuevos clasificados por Marca/Accesorio — Kino lo pidió sacar ese mismo día. Historial completo en MEMORIA_PROYECTO.md §9. **Pendiente:** sin test automatizado en `test/run.py` (el CSV real de Shopify no se subió al repo, es data de Kino); sin drag&drop (solo botón); sin tabla "antes → después" fila por fila para los que cruzan, solo resumen.

## Errores y trampas conocidas (resumen — detalle en MEMORIA_PROYECTO.md §8)

- **BUG GRAVE corregido 21 sep 2026:** el SYS real de Kino trae `Precio`/`Exist. unidades` como texto `"33,000.00"` (coma miles, punto decimal) — `aNum()` esperaba el formato al revés y esos precios se leían como **0** (43 filas con stock≥1.000 se descartaban enteras). El archivo de prueba no lo mostraba porque ahí son números nativos, no texto. Ya corregido (`aNum()` detecta el separador decimal por cuál aparece último) y verificado con el archivo real de Kino (9.993/3.133/246 exacto contra cálculo independiente, precios ya no en $0). **Si se subieron precios a Kyte/Shopify con un SYS así antes de este fix, pueden haber quedado en $0 — avisar a Kino para que revise.**
...
| Fecha | Tipo | Nota |
|---|---|---|
| 2026-09-21 | error | **Bug grave corregido, afecta toda la app (Kyte y Shopify):** `aNum()` no sabía leer `Precio`/`Exist. unidades` del SYS real de Kino, que vienen como texto "33,000.00" (coma miles, punto decimal — al revés de lo que esperaba). Resultado con su archivo real: todos los precios se leían como $0, y 43 productos con stock≥1.000 se descartaban enteros del cruce. El archivo de prueba no lo mostraba porque ahí esas columnas son números nativos, no texto. Kino lo reportó como "un tema con las columnas... para que coja realmente los valores". Corregido: `aNum()` ahora detecta el separador decimal por cuál aparece último (coma o punto) y trata el otro como separador de miles. Verificado con su archivo real: 9.993/3.133 con stock/246 negativos, exacto contra cálculo independiente; precios ya no en $0 (ej. "$260.000 → $250.000"). **Avisar a Kino: si ya subió precios a Kyte/Shopify con este SYS antes del fix, pueden haber quedado en $0 — conviene revisar.** |
| 2026-09-21 | error | Bug reportado por Kino con captura: "SKU nuevos" de Shopify traía productos de cualquier marca del SYS completo (SANSE, accesorios sueltos), no solo POEDAGAR. Corregido: `sysDeHoja()` acota el SYS a lo que también está en la hoja "POEDAGAR" de la tabla de Kyte (por Código, exacto+formato, 478 artículos) — única fuente para todo el sub-módulo, matching y "nuevos" incluidos. El mismo export real pasó de 2.660 "nuevos" (cualquier marca) a 53 (todos POEDAGAR reales), y "cruzan" de 532 a 418 (los otros 114 eran cruces con SYS de otras marcas, no debían contar). Si la tabla de Kyte no tiene hoja "POEDAGAR", el sub-módulo avisa y no procesa nada (`S.shop.sinHoja`). |
| 2026-09-21 | decisión | Confirmado por Kino: la regla de precio de Shopify POEDAGAR es "precio de Kyte (SYS×2) + 40%" = SYS×2,8 — no era un número suelto, coincide exacto con el 2.8 ya encontrado cruzando datos. El Excel que baja el sub-módulo de Shopify pasa a tener una sola hoja llamada "POEDAGAR ACTUALIZADA" (antes "Products"), y el archivo descargado también se llama así — es específico de este sub-módulo, no toca la convención de nombres de Kyte. |
| 2026-09-21 | avance | Sub-módulo Shopify: los SKU nuevos vuelven — ahora en lista en pantalla (SKU/Descripción/Stock/Precio propuesto/Marca, paginada) + botón "Bajar Excel con estas filas agregadas" (filas nuevas con SKU/Handle/Título/Stock/Precio cargados, `Published=false`/`Status=draft`, el resto vacío para completar a mano; campos técnicos copiados del 100% de los productos reales del archivo, no adivinados). El botón principal no cambia — nunca agrega filas solo. Verificado: Excel con nuevos da 3.201 filas (541+2.660), todo exacto contra cálculo independiente. |
| 2026-09-21 | decisión | Sub-módulo Shopify simplificado, a pedido de Kino: sacado el botón "Reporte (.xlsx)" con SKU nuevos (se había agregado ese mismo día). La descarga principal pasó de `.csv` a `.xlsx` — Kino convierte a CSV a mano cuando resube a Shopify. `procesarShopify()` volvió a la versión simple (sin `nuevos`/`familias`). |
| 2026-09-21 | avance | Sub-módulo Shopify: agregado botón "Reporte (.xlsx)" que marca los SKU nuevos (del SYS, sin usar en ese archivo de Shopify) con Marca/Accesorio/Familia — pedido explícito de Kino ("importante marcar los nuevos"). Reusa `clasificarNuevo()` ya construida para Kyte. Verificado: 2.660 SKU nuevos clasificados idéntico contra cálculo independiente en Python. (Revertido el mismo día — ver entrada más arriba.) |
