...
**Funciones en `sys-a-kyte.html`:** `leerShopify()` (ubica columnas `Variant SKU`/`Variant Inventory Qty`/`Variant Price` por nombre, conserva las 47 columnas originales tal cual), `procesarShopify()` (el cruce; también arma `res.nuevos`, los SYS sin usar en el archivo), `xlsxShopify()` (Excel de salida, sin filas nuevas), `slug()`/`filaNuevaShopify()`/`xlsxShopifyConNuevos()` (arman las filas nuevas), `renderShopify()`/`nuevosShopifyHTML()` (pantalla, reusa `badgesDir()` de las tarjetas de Kyte para los `↑`/`↓`).

**Verificado (21 sep 2026)** con un export real de Shopify (107 productos, 541 variantes/SKU) contra el SYS de prueba (`datos/INVENTARIO_KINO_18_de_sep.xlsx`): 532 de 541 SKU cruzaron, el Excel principal coincidió celda por celda (541 filas × 47 columnas) contra un cálculo independiente en Python, y el Excel "con nuevos" dio 3.201 filas (541 + 2.660 nuevas) donde las primeras 541 son idénticas al Excel principal y las 2.660 filas nuevas coinciden campo por campo (Handle/Title/SKU/Qty/Price/técnicos) contra el mismo cálculo independiente.
