...
4. **SKU nuevos.** Los del SYS que ningún nivel usó. Con stock mayor que 0: tarjeta destacada "SKU nuevos con stock: N · U uds", lista ordenada por unidades y hoja del reporte. Sugerencia de clasificación: marca por palabra clave (POEDAGAR, JOEFOX, CURREN, SANSE, KARDOO, SKMEI, T5, JAKCOM, LUGANOS, CASIO, DISITE, EXTRI), accesorio (ESTUCHE, CAJA, BOLSA, CORREA, PILA, TAPA, EXHIBIDOR), y "familia ya existe en tu tabla" (código sin el sufijo numérico final: `1401-3` → `1401`). Stock 0 y negativos: solo conteo. **Sin archivo aparte y sin agregarlos a ningún archivo.**
   > **Construido (21 sep 2026):** la clasificación (Marca / Accesorio / Familia ya en tu tabla) ya está en la hoja `Nuevos en SYS` del reporte, como 3 columnas nuevas — igual al criterio de arriba. Falta la tarjeta destacada en pantalla (Kino pidió por ahora solo el reporte). Funciones: `familiaCod()`, `clasificarNuevo()`, `MARCAS`/`ACCESORIOS` en `sys-a-kyte.html`.
...
- **BUG GRAVE corregido (21 sep 2026): `Precio` y `Exist. unidades` del SYS real vienen como texto "33,000.00"** (coma de miles, punto decimal) — no como número, y al revés del formato que `aNum()` esperaba (punto miles, coma decimal, estilo "1.234,56"). Con el archivo real de Kino (9.995 filas, todas con este formato): **todos los precios se leían como 0** (`aNum` daba `NaN` → se pisaba a 0 sin avisar) y **43 productos con stock ≥ 1.000 se descartaban enteros** del cruce (`NaN` en stock → fila inválida). El archivo de prueba (`datos/INVENTARIO_KINO_18_de_sep.xlsx`) no lo mostraba porque ahí `Precio`/`Exist. unidades` son números reales de Excel, no texto — por eso no se había visto antes. `aNum()` ahora detecta el separador decimal por cuál aparece último en el string (coma o punto) y borra el otro como separador de miles; con un solo separador y ≤2 dígitos después se asume decimal. Sigue funcionando para números ya nativos y para el formato viejo ("15000,50"). **Si algún precio actualizado antes de este fix con un SYS de formato texto ya se subió a Kyte o Shopify, revisarlo — puede haber quedado en $0.**
...
**Única fuente: la hoja "POEDAGAR" de la tabla de Kyte (confirmado por Kino, 21 sep 2026).** No se cruza contra los 9.993 artículos del SYS completo — solo contra los que también están en la hoja `POEDAGAR` de la tabla base de Kyte (478 en los datos de prueba), cruzados por Código con el mismo criterio exacto+formato que usa `procesar()` (`k1`/`k2`, formato válido solo si es único). Sin esto, "SKU nuevos" traía cualquier cosa del SYS (otras marcas — SANSE, XINJIA — y accesorios sueltos — TAPA, EXHIBIDOR), no solo lo que es POEDAGAR; con el corte a 478 de la hoja POEDAGAR, el mismo export real pasó de 2.660 a **53 SKU nuevos**, todos genuinamente POEDAGAR. Función: `sysDeHoja(tabla,sys,'POEDAGAR')` construye este SYS acotado; `calcularShopify()` la arma antes de cada `procesarShopify()`. Si la tabla de Kyte no tiene una hoja llamada exactamente "POEDAGAR", el sub-módulo avisa en pantalla y no deja subir nada (`S.shop.sinHoja`).

**Qué hace:** con ese SYS ya acotado a POEDAGAR, por cada fila del CSV de Shopify busca su `Variant SKU`:
- Si lo encuentra: `Variant Inventory Qty` = existencia del SYS (negativo → 0, igual que Kyte). Si el checkbox "Actualizar precio" está activo, `Variant Price` = redondeado(precio SYS × multiplicador).
- Si no lo encuentra (ni en el SYS acotado a POEDAGAR): la fila no se toca, se cuenta en "sin cruzar" (mismo principio que Kyte: "lo que no cruza no se toca"). Con el corte a POEDAGAR, "sin cruzar" subió de 9 a 123 sobre el mismo archivo — son Variant SKU que no están en la hoja POEDAGAR de Kyte (antes cruzaban igual contra el SYS completo, lo cual era el bug).

**Regla de precio, confirmada por Kino (21 sep 2026): Kyte siempre usa SYS × 2. Shopify POEDAGAR usa ese mismo precio de Kyte + 40 % — es decir SYS × 2 × 1,4 = SYS × 2,8.** Por eso el multiplicador por defecto en pantalla es 2.8 (coincide con lo que ya se había encontrado cruzando datos: 504 de 532 coincidencias de un export real daban exactamente ese número). **El multiplicador sigue siendo un número que carga Kino a mano** (input en pantalla, no fijo en el código) — si el 40% cambia algún día, el número a poner es `2 × (1 + nuevo%)`.
...
**Se sube CSV, se baja Excel — una sola hoja, llamada "POEDAGAR ACTUALIZADA".** Shopify solo exporta en CSV, así que la subida sigue siendo `.csv`. La descarga es `.xlsx` con una única hoja (`HOJA_SHOPIFY='POEDAGAR ACTUALIZADA'`, constante en `sys-a-kyte.html`) — Kino lo convierte a CSV a mano cuando lo necesita para resubir a Shopify. El nombre del archivo descargado también usa ese nombre: `POEDAGAR_ACTUALIZADA_<fecha>.xlsx` (o `..._con_nuevos_<fecha>.xlsx` para el que suma filas nuevas). Antes bajaba directo en `.csv` con la hoja sin nombre propio ("Products"); se cambió a pedido de Kino el 21 sep 2026 — "solo para este caso" (es decir, es específico de Shopify POEDAGAR, no de los archivos de Kyte).
...
**SKU nuevos (del SYS, sin usar en este archivo de Shopify): en pantalla + Excel aparte para completar a mano** (versión final del 21 sep 2026 — hubo un reporte descargable que se sacó y esto lo reemplaza, ver "Historial de este punto" abajo). Debajo de la tarjeta, si hay SKU nuevos con stock:
- Lista en pantalla (tabla SKU / Descripción / Stock / Precio propuesto / Marca), ordenada por stock, paginada de a 100 ("Mostrar más" — `S.shop.limite`). Es solo lectura, no descarga nada.
- Botón aparte **"Bajar Excel con estas filas agregadas"**: mismo Excel que el botón principal, más una fila nueva al final por cada SKU nuevo con stock. En esa fila se completan solos: `Handle` (SKU pasado a minúsculas/slug), `Title` (SKU + descripción del SYS), `Variant SKU`, `Variant Inventory Qty`, `Variant Price` (con el mismo multiplicador), y los campos técnicos que son **idénticos en el 100 % de los productos reales del archivo** (verificado, no adivinado): `Vendor=My Store`, `Option1 Name=Referencia`, `Variant Inventory Tracker=shopify`, `Variant Inventory Policy=deny`, `Variant Fulfillment Service=manual`, `Variant Requires Shipping=false`, `Variant Taxable=false`, `Variant Weight Unit=kg`, `Gift Card=false`. `Published` y `Status` se pisan a propósito a `false`/`draft` (borrador, no publicado) para que no se vea en la tienda hasta que Kino la complete. Todo lo demás (fotos, categoría, descripción larga, Option1 Value, SEO, etc.) queda vacío — es justamente lo que Kino completa a mano antes de subir.
- **Nunca se mezcla solo en el botón principal** ("Bajar Excel actualizado"): ese sigue bajando únicamente las filas que ya estaban en el archivo, sin agregar nada, como pide la regla general.

**Historial de este punto (todo el 21 sep 2026):** primero se agregó un reporte (.xlsx) aparte con hoja "Nuevos en SYS" clasificada por Marca/Accesorio/Familia (reusando `clasificarNuevo()`/`MARCAS`/`ACCESORIOS`/`familiaCod()`, construidas para el reporte de Kyte) — Kino lo pidió sacar. Después pidió verlos en pantalla y poder bajarlos como filas nuevas para completar a mano, que es la versión de arriba. La clasificación por Marca/Accesorio no se volvió a usar acá (si se quiere, son las mismas funciones de Kyte, ahí siguen).

**Funciones en `sys-a-kyte.html`:** `sysDeHoja(tabla,sys,nombreHoja)` (arma el SYS acotado a una hoja de Kyte por Código, ver arriba), `calcularShopify()` (orquesta: arma el SYS acotado a POEDAGAR y llama a `procesarShopify`, o marca `S.shop.sinHoja` si no hay hoja POEDAGAR), `leerShopify()` (ubica columnas `Variant SKU`/`Variant Inventory Qty`/`Variant Price` por nombre, conserva las 47 columnas originales tal cual), `procesarShopify()` (el cruce contra el SYS que le pasen — ya acotado; también arma `res.nuevos`), `xlsxShopify()` (Excel de salida, sin filas nuevas), `slug()`/`filaNuevaShopify()`/`xlsxShopifyConNuevos()` (arman las filas nuevas), `renderShopify()`/`nuevosShopifyHTML()` (pantalla, reusa `badgesDir()` de las tarjetas de Kyte para los `↑`/`↓`).

**Verificado (21 sep 2026)** con un export real de Shopify (107 productos, 541 variantes/SKU) contra el SYS de prueba (`datos/INVENTARIO_KINO_18_de_sep.xlsx`) **acotado a la hoja POEDAGAR** (478 artículos): 418 de 541 SKU cruzaron (123 sin cruzar), el Excel principal coincidió celda por celda (541 filas × 47 columnas) contra un cálculo independiente en Python que replica el mismo acotado (SYS ∩ Código de la hoja POEDAGAR, exacto+formato), y el Excel "con nuevos" dio 594 filas (541 + 53 nuevas, todas visiblemente POEDAGAR) en el mismo orden que el cálculo independiente. Antes de este acotado (mismo día, ver más arriba), sin restringir a la hoja POEDAGAR, daba 532 cruzan y 2.660 "nuevos" — la mayoría de otras marcas, ese era el bug que Kino reportó.

**Pendiente / no construido:** no quedó como test automatizado en `test/run.py` — el CSV real de Shopify no se subió al repo por ser datos propios de Kino, no de prueba. Si se toca `leerShopify`/`procesarShopify`/`xlsxShopify`/`filaNuevaShopify`, conviene rearmar la verificación a mano (subir un CSV real, comparar contra un cálculo independiente) antes de publicar. Tampoco hay drag&drop para el CSV de Shopify (solo botón), ni tabla "antes → después" fila por fila para los que sí cruzan (como la de Kyte) — solo el resumen en la tarjeta.
